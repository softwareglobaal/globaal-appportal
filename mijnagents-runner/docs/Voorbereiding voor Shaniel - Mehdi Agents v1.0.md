...
| B4 | **Live verkeersinfo voor de reistijdblokken** (nu: vrije rijtijd OSRM x filefactor per vertrekuur; code voor Google Routes staat klaar) | Mehdi, in console.cloud.google.com: (1) project kiezen of aanmaken, (2) Billing koppelen (Routes API is betalend na het gratis maandkrediet; onze ~40 ritten per week vallen daar ruim binnen), (3) APIs & Services > Library > "Routes API" > Enable, (4) APIs & Services > Credentials > Create credentials > API key, (5) op de sleutel: Application restrictions None, API restrictions > Restrict key > Routes API, (6) op de server: regel GOOGLE_ROUTES_KEY=<sleutel> toevoegen aan ~/appportal/.env. Daarna gebruikt de Agendawacht live verkeer vanzelf; het blok zegt dan "live verkeer Google" |
