...
- Afdeling Mail: een wacht per postvak die Mehdi koos (25-09-2026, "elk e-mailadres een agent"): `mail-hinv` (info@h-invest.be), `mail-mch` (mch@, van de Mac naar de VM), `mail-prive` (hotmail; de Mac levert de koppen uit Mail via `mac/hotmail_koppen.py`, Microsoft laat geen wachtwoord-login toe) en `mail-melo` (melodiebvba@gmail.com). `mail-info-harc` is uitgezet. Daarboven `mail-regisseur` (rapporteert aan De Regisseur, Mehdi hoogstens een bericht per dag om 08:00). Regels in `koppelingen/postvak.py`, postvakken en mappen in `werkwijze/mailwachten.json`. Draait met `/usr/bin/python3` omdat de venv geen yaml heeft.
