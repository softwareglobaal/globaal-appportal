# Zet het portaal als lokale MCP-server in de Claude desktop-app.
#
...
#
# Dit is dezelfde server als de connector in claude.ai; er is er maar een. Wie
# inlogt bepaalt wat hij ziet, niet hoe hij koppelt.
#
...
# Zonder token hoort de server 401 te geven. Dat is het bewijs dat we de juiste
# server aan de lijn hebben en niet een foutpagina of een blokkade.
try {
...
if ($code -eq 401) {
    Zeg "[3/5] Portaal bereikbaar (401 zonder token, zoals verwacht)."
} elseif ($null -eq $code) {
    Fout "Portaal niet bereikbaar vanaf deze PC. Controleer de internetverbinding."
    exit 1
} else {
    Fout "Portaal antwoordt met $code in plaats van 401. Niet doorgaan, eerst uitzoeken."
    exit 1
}
...
Zeg ""
Zeg "  1. Start Claude. De eerste keer duurt het een halve minuut: de brug"
Zeg "     wordt dan opgehaald."
Zeg "  2. Er opent een browser met de inlogpagina. Laat HEM inloggen, als zichzelf."
Zeg "     Er wordt ALTIJD om een wachtwoord gevraagd, ook als er al iemand is"
Zeg "     ingelogd in die browser. Dat is expres: zo kan hij niet per ongeluk"
Zeg "     koppelen als de collega die daar nog een sessie had openstaan."
Zeg "  3. Controle in de chat: vraag 'welke applicaties mag ik lezen'."
Zeg "     Het antwoord begint met de naam waaronder hij gekoppeld is."
Zeg ""
Zeg "Krijgt hij bij het inloggen een 403: dan zit hij nog niet in de groep"
Zeg "'portaal-mcp'. Dat zet een beheerder recht in Authentik."
Zeg ""
