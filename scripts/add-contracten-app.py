...
grp, _ = Group.objects.get_or_create(name="contracten")

defaults = dict(authorization_flow=auth_flow, mode="forward_single",
                external_host=f"https://contracten.{BASE}")
if inval_flow:
    defaults["invalidation_flow"] = inval_flow
proxy, _ = ProxyProvider.objects.get_or_create(name="contracten-proxy", defaults=defaults)
proxy.external_host = f"https://contracten.{BASE}"
proxy.set_oauth_defaults()
proxy.save()

app, _ = Application.objects.get_or_create(
    slug="contracten",
    defaults=dict(name="Contracten", provider=proxy,
                  meta_launch_url=f"https://contracten.{BASE}"))
app.provider = proxy
app.meta_launch_url = f"https://contracten.{BASE}"
app.save()
...
for uname in ("mehdi", "akadmin"):
    ...
    if u:
        u.groups.add(grp)
        print(f"USER {uname} -> groep contracten")
    else:
        print(f"USER {uname} niet gevonden (sla over)")
print("CONTRACTEN_DONE")
