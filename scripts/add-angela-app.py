"""Registreer de Angela-tegel in Authentik (forward-auth proxy-provider).

Draaien (vanuit ~/appportal):
  sh scripts/ak-exec.sh scripts/add-angela-app.py

Idempotent: veilig om opnieuw te draaien. Maakt de tegel 'Angela' in de launcher,
toegankelijk voor de groepen admin/manager/angela. De app leest de gebruiker uit
de X-authentik-* headers; bewerken is niet extra afgeschermd (EDITOR_GROUPS
leeg), dus iedereen die de tegel mag openen mag ook droppen.
"""
import os
...
outpost.save()
print("embedded outpost: angela-proxy toegevoegd")
print("ANGELA_APP_DONE")
