"""Registreer de WhatsApp-tegel in Authentik (forward-auth proxy-provider).

Draaien (vanuit ~/appportal):
  sh scripts/ak-exec.sh scripts/add-whatsapp-app.py

Idempotent. Groep `whatsapp` = wie berichten behandelt en verstuurt (Office);
`whatsapp-beheer` = wie het nummerregister wijzigt. De webhook /twilio/ staat
in de vhost buiten deze provider om.
"""
import os
...
SLUG = "whatsapp"
NAME = "WhatsApp"
ROLES = ("admin", "manager", "whatsapp", "whatsapp-beheer")
...
proxy, created = ProxyProvider.objects.get_or_create(name=f"{SLUG}-proxy", defaults=proxy_defaults)
...
app.meta_description = (
    "WhatsApp van de firma's als gedeelde inbox. De agent stelt een antwoord "
    "voor, een collega verstuurt."
)
app.save()
...
outpost.save()
print("embedded outpost: whatsapp-proxy toegevoegd")
print("WHATSAPP_APP_DONE")
