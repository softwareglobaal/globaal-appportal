"""Registreer de Boekhouding-tegel in Authentik (forward-auth proxy-provider).

Draaien (vanuit ~/appportal):
  sh scripts/ak-exec.sh scripts/add-boekhouding-app.py

Idempotent: veilig om opnieuw te draaien. Maakt de tegel 'Boekhouding' in de
launcher, toegankelijk voor de groepen admin/manager/boekhouding. De app leest
de gebruiker uit de X-authentik-* headers en controleert zelf nog eens op de
groep, zodat een verkeerd gezette binding niet meteen toegang geeft.
"""
import os
...
outpost.save()
print("embedded outpost: boekhouding-proxy toegevoegd")
print("BOEKHOUDING_APP_DONE")
